...
// 数据目录可用 HABITPET_DATA_DIR 覆盖（测试启动器指向临时目录，避免污染真实数据）
const DATA_DIR = process.env.HABITPET_DATA_DIR || path.join(__dirname, '..', 'data');
...
function migrate(d) {
  ...
  for (const f of Object.values(d.families || {})) {
    ...
    for (const c of Object.values(f.children || {})) {
      ...
      // schema v5: 7天微习惯计划（引导层，不发分不加经验）+ 温和回归提示
      if (!c.onboarding || typeof c.onboarding !== 'object') {
        c.onboarding = { status: 'not_started', planId: null, startedOn: null, completedOn: [], completedAt: null, finishedOn: null, lastDismissedOn: null, history: [] };
      }
      const ob = c.onboarding;
      if (ob.status === 'idle') ob.status = 'not_started';   // 早期草稿命名，显式映射
      if (['not_started', 'active', 'completed', 'expired'].indexOf(ob.status) < 0) ob.status = 'not_started';
      if (['homework', 'reading', 'prepare'].indexOf(ob.planId) < 0) ob.planId = null;
      if (typeof ob.startedOn !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(ob.startedOn)) ob.startedOn = null;
      if (!Array.isArray(ob.completedOn)) ob.completedOn = [];
      ob.completedOn = [...new Set(ob.completedOn.filter(x => typeof x === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(x)))].sort();
      if (ob.completedAt !== null && typeof ob.completedAt !== 'string') ob.completedAt = null;
      if (ob.finishedOn !== null && typeof ob.finishedOn !== 'string') ob.finishedOn = null;
      if (ob.lastDismissedOn !== null && typeof ob.lastDismissedOn !== 'string') ob.lastDismissedOn = null;
      if (!Array.isArray(ob.history)) ob.history = [];
      ob.history = ob.history.filter(h => h && typeof h === 'object' && h.planId).slice(-3);
      if (ob.status === 'not_started') { ob.planId = null; ob.startedOn = null; }
      // schema v6: 回归提示补 pendingGapKey（服务端当前待确认的 gap，旧 key 永不吞新提示）
      if (!c.returnNudge || typeof c.returnNudge !== 'object') c.returnNudge = { lastShownForGap: null, pendingGapKey: null };
      if (c.returnNudge.lastShownForGap !== null && typeof c.returnNudge.lastShownForGap !== 'string') c.returnNudge.lastShownForGap = null;
      if (c.returnNudge.pendingGapKey !== null && typeof c.returnNudge.pendingGapKey !== 'string') c.returnNudge.pendingGapKey = null;
      // schema v7: 星图冒险（每日一次、非数值探索记录）
      if (!c.adventure || typeof c.adventure !== 'object') c.adventure = {};
      if (typeof c.adventure.chapterId !== 'string') c.adventure.chapterId = 'first-starlight';
      if (!Number.isInteger(c.adventure.chapterDay) || c.adventure.chapterDay < 1 || c.adventure.chapterDay > 7) c.adventure.chapterDay = 1;
      if (!Array.isArray(c.adventure.completedDays)) c.adventure.completedDays = [];
      c.adventure.completedDays = [...new Set(c.adventure.completedDays.filter(x => Number.isInteger(x) && x >= 1 && x <= 7))].sort((a, b) => a - b);
      if (c.adventure.chapterCompletedAt !== null && typeof c.adventure.chapterCompletedAt !== 'string') c.adventure.chapterCompletedAt = null;
      if (!Array.isArray(c.adventure.chapterCompletions)) c.adventure.chapterCompletions = [];
      c.adventure.chapterCompletions = c.adventure.chapterCompletions.filter(x => x && typeof x === 'object' && typeof x.completedAt === 'string').slice(-5);
      if (typeof c.adventure.lastPlayedOn !== 'string') c.adventure.lastPlayedOn = null;
      if (typeof c.adventure.currentNodeId !== 'string') c.adventure.currentNodeId = null;
      if (!Array.isArray(c.adventure.visited)) c.adventure.visited = [];
      if (!Array.isArray(c.adventure.discoveries)) c.adventure.discoveries = [];
      if (!Array.isArray(c.adventure.signatureEvents)) c.adventure.signatureEvents = [];
      if (!Array.isArray(c.adventure.randomEvents)) c.adventure.randomEvents = [];
      if (c.adventure.weeklyQuest !== null && typeof c.adventure.weeklyQuest !== 'object') c.adventure.weeklyQuest = null;
      if (c.adventure.lastResult !== null && typeof c.adventure.lastResult !== 'object') c.adventure.lastResult = null;
      c.adventure.visited = [...new Set(c.adventure.visited.filter(x => typeof x === 'string'))].slice(-30);
      c.adventure.discoveries = c.adventure.discoveries.filter(x => x && typeof x === 'object' && x.nodeId).slice(-30);
      c.adventure.signatureEvents = c.adventure.signatureEvents.filter(x => typeof x === 'string' || (x && typeof x === 'object')).slice(-30);
      c.adventure.randomEvents = c.adventure.randomEvents.filter(x => x && typeof x === 'object' && x.id).slice(-30);
      if (c.adventure.completedDays.length && c.adventure.chapterDay <= c.adventure.completedDays[c.adventure.completedDays.length - 1]) c.adventure.chapterDay = Math.min(7, c.adventure.completedDays[c.adventure.completedDays.length - 1] + 1);
      if (c.adventure.completedDays.length >= 7 && !c.adventure.chapterCompletedAt) c.adventure.chapterCompletedAt = new Date().toISOString();
    }
...
function addChild(family, name, grade) {
  ...
  family.children[id] = {
    ...
    adventure: { chapterId: 'first-starlight', chapterDay: 1, completedDays: [], chapterCompletedAt: null, chapterCompletions: [], lastPlayedOn: null, currentNodeId: null, visited: [], discoveries: [], signatureEvents: [], randomEvents: [], weeklyQuest: null, lastResult: null }
  };
  save();
